...
hidden_layers = [1024 for i in range(20)]
prefix='prune2'
...
train_dataset = BatchedFakeParticleDataset_All(batch_size=train_batch_size, length=100_000)

train_dataloader = DataLoader(train_dataset, batch_size=None, shuffle=False, num_workers=5)

val_dataset = BatchedFakeParticleDataset_All(batch_size=10000, length=100_000)
...
class CustomKinematicNetWithPruning(CustomKinematicNet):
    def __init__(self, input_size, hidden_layers, lenoutput, activation_fn=F.relu):
        super().__init__(input_size, hidden_layers, lenoutput, activation_fn)
        
    def prune_neurons(self, percentage):
        new_layers = []
        for idx, layer in enumerate(self.layers):
            if isinstance(layer, nn.Linear):  # Pruning only Linear layers
                neuron_l1_norm = layer.weight.data.norm(p=1, dim=1)
                threshold = torch.quantile(neuron_l1_norm, percentage)
                mask = neuron_l1_norm >= threshold
                
                pruned_weight = layer.weight.data[mask]
                pruned_bias = layer.bias.data[mask] if layer.bias is not None else None
                
                if (idx % 3 == 0) and (idx != 0):
                    in_features = pruned_weight.shape[0] + self.input_size
                else:
                    in_features = pruned_weight.shape[0]
                
                new_layer = nn.Linear(in_features, layer.out_features)
                new_layer.weight.data = pruned_weight
                if pruned_bias is not None:
                    new_layer.bias.data = pruned_bias
    ...
    def forward(self, x):
        inputs = x
        for idx, layer in enumerate(self.layers[:-1]):
            # Calculate the in_features for the current layer based on its weight matrix
            in_features = layer.weight.data.shape[1]
            # print("in_features: ", in_features)
            
            if (idx % 3 == 0) and (idx != 0):
                x = self.activation_fn(layer(torch.cat((x[:, :in_features - self.input_size], inputs), dim=-1)))
            else:
                x = self.activation_fn(layer(x[:, :in_features]))
        
        # Calculate the in_features for the final layer based on its weight matrix
        in_features = self.layers[-1].weight.data.shape[1]
        
        # if (len(self.layers) - 1) % 3 == 0:
        #     return self.layers[-1](torch.cat((x[:, :in_features - self.input_size], inputs), dim=-1))
        return self.layers[-1](x[:, :in_features])
...
def calculate_sparsity(model):
    ...
    for layer in model.layers:
        if isinstance(layer, nn.Linear):  # You can extend this for other types of layers
            total_elements += torch.numel(layer.weight.data)
            zero_elements += torch.sum(layer.weight.data == 0).item()
    
    sparsity = (zero_elements / total_elements) * 100  # Calculate sparsity in percentage
    return sparsity

# %%
model = CustomKinematicNetWithPruning(input_dim, hidden_layers, output_dim, activation_fn=activation)
...
scheduler = torch.optim.lr_scheduler.ExponentialLR(optimizer, gamma=0.97, last_epoch=-1, verbose=False)
