...
def find_ticket_hypothesis(model, name, train_loader, test_loader, start_iter = 0, end_iter=6, num_epochs=50, learning_rate=0.0004, prune_amount=0.1, k=3):
    ...
    try:
        ...
        train(model, train_loader, test_loader, num_epochs = k, optimizer=optimizer, scheduler=scheduler )  #Save Kth epoch model
    ...
    train(model, train_loader, test_loader, num_epochs = num_epochs-k, optimizer=optimizer , scheduler=scheduler)  #Train for remaining epochs
    ...
    for i in range(start_iter, end_iter):
        ...
        print("global sparsity: {:.2f}%".format(calculate_sparsity(model, totalweights)))

        #Train
        optimizer=torch.optim.Adam(model.parameters(), lr=learning_rate)
        train(model, train_loader, test_loader, num_epochs = num_epochs, optimizer=optimizer, scheduler=scheduler)
...
find_ticket_hypothesis(model, prefix, train_dataloader, val_dataloader, end_iter=30, num_epochs=50, learning_rate=lr_start, prune_amount=0.1, k=3)
