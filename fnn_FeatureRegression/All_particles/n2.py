...
def custom_loss(y_pred, y_true, output_dim = output_dim, clip_value=50.0):
    ...
    log_cosh_loss = torch.log(torch.cosh(y_pred - y_true) + 1e-12)  # small constant added for numerical stability
    
    num_features = int(output_dim)
    
    # Relative log cosh loss for the last feature
    relative_term = (y_pred[:, num_features - 1] - y_true[:, num_features - 1]) / (y_true[:, num_features - 1] + 1e-12)  # small constant added for numerical stability
    relative_log_cosh_loss_last_feature = torch.log(torch.cosh(relative_term) + 1e-12)  # small constant added for numerical stability
