...
# .path.append('/home/ddemler/HNLclassifier/fnn_FeatureRegression/All_particles')
...
# train_dataset = BatchedFakeParticleDataset_All(batch_size=train_batch_size, length=200_000)

train_dataloader = DataLoader(train_dataset, batch_size=None, shuffle=False, num_workers=5)

val_dataset = BatchedFakeParticleDataset_All(batch_size=10000, length=1_000_000)
# val_dataset = BatchedFakeParticleDataset_All(batch_size=10000, length=200_000)
...
class CustomKinematicNet(nn.Module):
    ...
    def parameters_to_prune(self):
        return [
            (module, 'weight') for module in self.layers[1:-1] if isinstance(module, nn.Linear)
        ]

    

# class CustomKinematicNetWithPruning(CustomKinematicNet):
#     def prune_neurons(self, percentage):
#         for idx, layer in enumerate(self.layers):
#             if isinstance(layer, nn.Linear):
#                 # Compute L1 norm for each neuron
#                 neuron_l1_norm = layer.weight.data.norm(p=1, dim=1)
                
#                 # Determine the threshold for pruning
#                 threshold = torch.quantile(neuron_l1_norm, percentage)
                
#                 # Create a mask for neurons we want to keep
#                 mask = neuron_l1_norm >= threshold
                
#                 # Prune neurons
#                 pruned_weight = layer.weight.data[mask]
#                 pruned_bias = None
#                 if layer.bias is not None:
#                     pruned_bias = layer.bias.data[mask]
                
#                 # Create a new layer to replace the old one
#                 new_layer = nn.Linear(pruned_weight.shape[0], layer.out_features)
#                 new_layer.weight.data = pruned_weight
#                 if pruned_bias is not None:
#                     new_layer.bias.data = pruned_bias
                
#                 # Replace the layer in the list of layers
...
def calculate_sparsity(model, totalweights):
    total_zeros=0
    for layer in model.layers:
        if isinstance(layer, nn.Linear):
            total_zeros+=torch.sum(layer.weight==0)
    # print(total_zeros)
    return (total_zeros/totalweights)*100
 
    
def train(model, train_dataloader, val_dataloader, num_epochs, optimizer,scheduler, patience_model=15):
    min_valloss=np.inf
    epoch_loss_list=[]
    for epoch in range(num_epochs):
        ...
        with torch.no_grad():
            ...
            for batch_idx, (data, target) in enumerate(val_dataloader):
                ...
                target = target.to(device)
                outputs = model(data)
                _, loss = custom_loss(outputs, target)
                val_loss += loss.item()
        ...
        print("Epoch: {}, Training Loss: {:.4e}, Validation Loss: {:.4e}, LR: {:.2e}".format(epoch, total_loss, val_loss, old_lr))
        epoch_loss_list.append(val_loss)
        scheduler.step(val_loss)
    return epoch_loss_list

            # elif val_loss > min_valloss*1e3:
...
def find_ticket_hypothesis(model, name, train_loader, test_loader, start_iter = 0, end_iter=6, num_epochs=50, learning_rate=0.001, prune_amount=0.1, k=3):

    losses_pd=pd.DataFrame()

    optimizer = torch.optim.Adam(model.parameters(), lr=learning_rate)
    # scheduler = torch.optim.lr_scheduler.ExponentialLR(optimizer, gamma=lr_gamma, last_epoch=-1, verbose=False)
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, 'min', patience=lr_patience, factor=0.7, verbose=True)

    # try:
    #     model.load_state_dict('/home/ddemler/HNLclassifier/saved_files/saved_models/lottery_ticket/' + name + '_RewindWeights'+'_' + str(k) + '.pt')
    #     print("loaded model")
    # except:
    llist1= train(model, train_loader, test_loader, num_epochs = k, optimizer=optimizer, scheduler=scheduler )  #Save Kth epoch model
    # losses_pd['original']=llist
    torch.save(model.state_dict(), '/home/ddemler/HNLclassifier/saved_files/saved_models/lottery_ticket/' + name + '_RewindWeights'+'_' + str(k) + '.pt')

    model_rewind = CustomKinematicNet(input_dim, hidden_layers, output_dim, activation_fn=activation)
    model_rewind.load_state_dict(torch.load('/home/ddemler/HNLclassifier/saved_files/saved_models/lottery_ticket/' + name + '_RewindWeights'+'_' + str(k) + '.pt'))

    llist2= train(model, train_loader, test_loader, num_epochs = num_epochs-k, optimizer=optimizer , scheduler=scheduler)  #Train for remaining epochs
    llist=llist1+llist2
    losses_pd['original']=llist.reverse()
    print("length of losses_pd: ", len(losses_pd['original']))
    # losses_pd['original'].
    ...
    for layer in model.layers:
        if isinstance(layer, nn.Linear):
            totalweights+=layer.weight.nelement()

    for i in range(start_iter, end_iter):

        # prune.global_unstructured(model.parameters)
        ...
        curr_sparsity=calculate_sparsity(model, totalweights).item()
        print("global sparsity: {:.2f}%".format(curr_sparsity))

        #Train
        curroptimizer=torch.optim.Adam(model.parameters(), lr=learning_rate)
        # scheduler = torch.optim.lr_scheduler.ExponentialLR(curroptimizer, gamma=lr_gamma, last_epoch=-1, verbose=False)
        scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(curroptimizer, 'min', patience=lr_patience, factor=0.7, verbose=True)
        llist = train(model, train_loader, test_loader, num_epochs = num_epochs, optimizer=curroptimizer, scheduler=scheduler)
        # print("length of llist: ", len(llist))

        losses.append(test(model, test_loader))
        losses_pd[str(curr_sparsity)+ '%_sparsity'] = llist.reverse()

        plt.plot(np.arange(len(losses)), losses)
        plt.xlabel("Pruning Iteration")
        plt.ylabel("Test Loss")
        plt.savefig('/home/ddemler/HNLclassifier/saved_files/saved_models/lottery_ticket/' + name + '_losses.png')
        plt.close()
        print("losses: ", losses)
        torch.save(model.state_dict(), '/home/ddemler/HNLclassifier/saved_files/saved_models/lottery_ticket/saved_iter/' + name + '_pruned_weights_' + str(i+1) + '.pt')
        losses_pd.to_csv('/home/ddemler/HNLclassifier/saved_files/saved_models/lottery_ticket/' + name + '_losses.csv', index=False)
...
# train(model, train_dataloader, val_dataloader, num_epochs=10, optimizer=optimizer, savepath=modelsavepath, scheduler=scheduler, patience_model=patience_model)

find_ticket_hypothesis(model, prefix, train_dataloader, val_dataloader, end_iter=30, num_epochs=200, learning_rate=lr_start, prune_amount=0.10, k=3)
...
# parameters_to_prune = [
#     (module, 'weight') for module in model.layers if isinstance(module, nn.Linear)
#  ]
...
#         print("Epoch: {}, Training Loss: {:.4e}, Validation Loss: {:.4e}, LR: {:.2e}, Sparsity: {:.2f}%".format(epoch, total_loss, val_loss, old_lr, sparsity))

#         if epoch % 10 == 0 and epoch >0 and sparsity <90:
...
# def load_pruned_model(model, state_dict):
#     """
#     Load a pruned model by resizing its layers to fit the pruned state dict.
#     """
#     new_layers = []
#     for idx, (layer, (name, pruned_weight)) in enumerate(zip(model.layers, state_dict.items())):
#         if "layers." in name and ".weight" in name:
#             new_layer = nn.Linear(pruned_weight.shape[1], pruned_weight.shape[0])
#             new_layer.weight.data = pruned_weight
#             new_layer.bias.data = state_dict[name.replace(".weight", ".bias")]
#             new_layers.append(new_layer)
#         elif idx == 0:  # Copy the first layer if not pruned
...
# # model=CustomKinematicNet(input_dim, hidden_layers, output_dim, activation_fn=activation)
# model = savedmodel
# # pruned_state_dict= torch.load(modelsavepath)
# # load_pruned_model(model, pruned_state_dict)
# model.to(device)
