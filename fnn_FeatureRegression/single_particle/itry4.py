...
hidden_layers = [12 for i in range(10)]
prefix='itry4'

#! copied from itry 3. Different input distribution
...
class FakeParticleDataset(Dataset):
    ...
    def __getitem__(self, idx):
        phi, eta, mass, pt = self.generate_input_data_biased_towards_zero()
    ...
    def generate_input_data(self):
        ...
        mass_low, mass_high = 0.1, 1000
        phi_low, phi_high = -np.pi, np.pi
        pt_low, pt_high = 0.1, 1000
    ...
    def generate_input_data_biased_towards_zero(self):
        eta_low, eta_high = -2.5, 2.5
        mass_low, mass_high = 0.1, 1000
        phi_low, phi_high = -np.pi, np.pi
        pt_low, pt_high = 0.1, 1000

        # Generate a random number to decide if we should sample using the biased method
        use_biased_sampling = np.random.rand() < 0.5  # 50% chance of using biased sampling
        
        if use_biased_sampling:
            # Using beta distribution to bias towards 0
            # Adjusting the parameters of the beta distribution can change the bias strength
            bias_strength = 0.5  # This can be adjusted
            eta = (np.random.beta(bias_strength, bias_strength) - 0.5) * (eta_high - eta_low)
            mass = np.random.beta(bias_strength, bias_strength) * (mass_high - mass_low)
            phi = (np.random.beta(bias_strength, bias_strength) - 0.5) * (phi_high - phi_low)
            pt = np.random.beta(bias_strength, bias_strength) * (pt_high - pt_low)
        else:
...
dfcols=["train_loss","val_loss",'phi', 'eta', 'mass', 'pt']
